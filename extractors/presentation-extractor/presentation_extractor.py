...
Author Alan O'Cais <alan.ocais@cecam.org>
"""
...
def create_video_previews(filename, output_dir, mp4_filename, webm_filename, webm):
    ...
    if webm:
        ffmpeg_command = (
            ffmpeg_stub + webm_settings + no_audio + "-pass 1 -f webm /dev/null"
        )
        subprocess.check_output(ffmpeg_command, stderr=subprocess.STDOUT, shell=True)
        ffmpeg_command = (
            ffmpeg_stub
...
class VideoMetaData(Extractor):
    ...
    def read_settings(self, filename=None):
        ...
        try:
            with open(filename, "r") as settingsfile:
                ...
                self.mask_settings = settings.get("masks", [])
                algorithm_settings = settings.get("slides")
                self.algorithm_settings = (
                    algorithm_settings[0] if algorithm_settings else {}
                )
    ...
    def process_message(self, connector, host, secret_key, resource, parameters):
        ...
        user_mask = usersettings.get("masks")
        if isinstance(user_mask, (dict, list)):
            self.mask_settings = user_mask

        user_slides = usersettings.get("slides")
        if isinstance(user_slides, dict):
            self.algorithm_settings.update(user_slides)

        self.tempdir = tempfile.mkdtemp(prefix="clowder-video-presentation")

        self.find_slides_transitions(
            connector, host, secret_key, resource, masks=self.mask_settings, webm=False
        )
    ...
    def generate_vtt_chapters(self):
        ...
        prev_time = datetime.datetime.utcfromtimestamp(0) + datetime.timedelta(
            milliseconds=self.results[0][1]
        )
        ...
        for idx, (_, time_idx) in enumerate(self.results[1:]):
            begin_delta = datetime.datetime.utcfromtimestamp(0) + datetime.timedelta(
                milliseconds=time_idx
            )
            # microseconds always get printed as 6 digits passed with zeros, so we delete the last 3 digits
            vttfile.append(
                "%s --> %s"
                % (
                    prev_time.strftime(format_str)[:-3],
                    begin_delta.strftime(format_str)[:-3],
                )
            )
    ...
    def prepare_masks(self, masks, frame):
        ...
        for mask in masks:
            ...
            if (
                len(where) != 2
                or where[0] not in locations_vert
                or where[1] not in locations_hori
            ):
                self.logger.error(
                    "Invalid location setting: %s. Possible choices: %s",
                    mask.get("location"),
                    ", ".join(
                        [
                            "%s-%s" % (y, x)
                            for x in locations_hori
                            for y in locations_vert
                        ]
                    ),
                )
                continue
            ...
            if where[0] == "bottom":
                y2 = frame[0]
                y1 = frame[0] - parse_value(mask.get("size_y", 0), frame[0])
            else:
                y1 = 0
                y2 = parse_value(mask.get("size_y", 0), frame[0])

            if where[1] == "right":
                x2 = frame[1]
                x1 = frame[1] - parse_value(mask.get("size_x", 0), frame[1])
            else:
                x1 = 0
                x2 = parse_value(mask.get("size_x", 0), frame[1])
    ...
    def find_slides_transitions(
        self, connector, host, secret_key, resource, masks=None, webm=False
    ):  # pylint: disable=unused-argument,too-many-arguments
        """Find slides"""
        ...
        if self.algorithm_settings.get("algorithm", "") == "basic":
            settings = dict(default_settings_basic)  # make sure it's a copy
            settings.update(
                dict(
                    [
                        (key, self.algorithm_settings[key])
                        for key in self.algorithm_settings
                        if key in default_settings_basic.keys()
                    ]
                )
            )
            self.logger.debug(
                "Using basic algorithm for finding slides. settings: %s", settings
            )
            results = self.slide_find_basic(
            ...
            settings.update(
                dict(
                    [
                        (key, self.algorithm_settings[key])
                        for key in self.algorithm_settings
                        if key in default_settings_advanced.keys()
                    ]
                )
            )
            self.logger.info(
        ...
        if os.path.exists(mp4_preview_file):
            mp4_preview_id = self.try_upload_preview_file(
            ...
            if webm and os.path.exists(webm_preview_file):
                webm_preview_id = self.try_upload_preview_file(
                    pyclowder.files.upload_preview,
                    connector,
                    host,
                    secret_key,
        ...
        slidesmeta = {
            "nrslides": 0,
            "listslides": [],
            "algorithm": self.algorithm_settings.get("algorithm", "advanced"),
            "settings": settings,
            "previews": previews,
        }
        ...
        for idx, (frame_idx, time_idx, slidepath) in enumerate(results):
            ...
            previewid = self.try_upload_preview_file(
                pyclowder.files.upload_preview,
                connector,
                host,
                secret_key,
        ...
        if self.results and len(self.results) > 1:
            slidesmeta["nrslides"] = (
                len(self.results) - 1,
            )  # the last frame always gets added too
            ...
            for _, time_idx, new_previewid in self.results[1:]:
                begin_delta = datetime.datetime.utcfromtimestamp(
                    0
                ) + datetime.timedelta(milliseconds=time_idx)
                # microseconds always get printed as 6 digits passed with zeros, so we delete the last 3 digits
                slidesmeta["listslides"].append(
                    (
                        str(prev_time.strftime(format_str)[:-3]),
                        str(begin_delta.strftime(format_str)[:-3]),
                        str(previewid),
                        str(prev_time_msec / 1000),
                    )
                )
    ...
    def slide_find_advanced(self, filename, connector, resource, **settings):
        ...
        while frame_index < num_frames:
            ...
            if (frame_index % round(num_frames / 100.0)) == 0:
                ...
                if percent_processed % 10 == 0 or percent_processed == 99:
        ...
        for slide in slides:
            ...
            cap.set(cv2.CAP_PROP_POS_MSEC, min(slide[1] + msec_to_delay_screenshot, final_timestamp))
            # Grab the image
            _, frame = cap.read()
            # Save the image
    ...
    def slide_find_basic(self, filename, **kwargs):  # pylint: disable=too-many-locals
        ...
        )  # assume it's constant and we convert to integer
        ...
        while True:
            ...
            _, frame_thres = cv2.threshold(
                frame_diff, threshold_cutoff, 255, cv2.THRESH_BINARY
            )
            ...
            if frame_idx % (10 * fps) == 0:
                self.logger.debug(
                    "Slide transition detection %.2f%% done\t%s",
                    float(frame_idx) / nFrames * 100,
                    time_real,
                )
